    ...
    if (Config.EnableGPSUSB) {
        if (Config.GPSUSBPort) {
            if (connectUSB(Config.GPSUSBPort, Config.GPSUSBBaudrate) > 0) {
                ...
                while ( true )
                {
                    static bool telemetry_rx = false;
                    if (Config.GPSUSBOutputFreq == 0) {
                        dequeued_telemetry_ptr = lifo_buffer_waitpop(&GPS_USB_Upload_Buffer);
                    } else {
                        dequeued_telemetry_ptr = lifo_buffer_pop(&GPS_USB_Upload_Buffer);
                    }
                    ...
                    if(dequeued_telemetry_ptr != NULL)
                    {
                        ...
                        if (Config.GPSUSBOutputFreq == 0) {
                            // Send data as soon as received if no configured freq
                            gpsUsbSendFunc();
                        }
                    } else {
                        if (Config.GPSUSBOutputFreq == 0) {
                            /* NULL returned from lifo_buffer_waitpop: We've been asked to quit */
                            /* Don't bother free()ing stuff, as application is quitting */
                            break;
                        } else if (telemetry_rx) {
                            /* Send data with the specified frequency if configured so and any telemetry received */
                            sleep(1000 / Config.GPSUSBOutputFreq);
                            gpsUsbSendFunc();
                        }
                    }
                }
